...
func TestPublishRequestValidateAcceptsDraftStandardRequest(t *testing.T) {
	now := shanghaiTime(2026, 4, 10, 12, 0, 0)
	request := PublishRequest{
		Account:    "creator-a",
		Title:      "标题",
		Content:    "正文",
		Mode:       PublishModeDraft,
...
func TestPublishRequestValidateRequiresImageForStandardRequest(t *testing.T) {
	request := PublishRequest{Account: "creator-a", Title: "标题", Content: "正文", Mode: PublishModeDraft, MediaKind: MediaKindStandard}
...
func TestPublishRequestValidateRejectsDraftWithScheduleTime(t *testing.T) {
	now := shanghaiTime(2026, 4, 10, 12, 0, 0)
	scheduledAt := shanghaiTime(2026, 4, 10, 12, 20, 0)
	request := PublishRequest{Account: "creator-a", Title: "标题", Content: "正文", Mode: PublishModeDraft, ScheduleTime: &scheduledAt, MediaKind: MediaKindStandard, ImagePaths: []string{"cover.jpg"}}
	if err := request.Validate(now); err == nil || !strings.Contains(err.Error(), "draft mode forbids schedule time") {
		t.Fatalf("Validate() error = %v", err)
	}
}
...
func TestPublishRequestValidateRequiresLiveReportPath(t *testing.T) {
	request := PublishRequest{Account: "creator-live", Title: "标题", Content: "正文", Mode: PublishModeDraft, MediaKind: MediaKindLive}
...
func TestPublishRequestValidateRequiresResolvedLiveSource(t *testing.T) {
	request := PublishRequest{Account: "creator-live", Title: "标题", Content: "正文", Mode: PublishModeDraft, MediaKind: MediaKindLive, Live: LivePublishSource{ReportPath: "report.json"}}
...
func TestPublishRequestValidateRequiresResolvedLiveItems(t *testing.T) {
	request := PublishRequest{Account: "creator-live", Title: "标题", Content: "正文", Mode: PublishModeDraft, MediaKind: MediaKindLive, Live: LivePublishSource{ReportPath: "report.json", Resolved: &ResolvedLiveSource{}}}
...
func TestPublishRequestValidateRequiresAccount(t *testing.T) {
	request := PublishRequest{Title: "标题", Content: "正文", Mode: PublishModeDraft, MediaKind: MediaKindStandard, ImagePaths: []string{"cover.jpg"}}
...
func TestPublishRequestValidateRequiresTitleAndContent(t *testing.T) {
	now := shanghaiTime(2026, 4, 10, 12, 0, 0)
	missingTitle := PublishRequest{Account: "creator-a", Content: "正文", Mode: PublishModeDraft, MediaKind: MediaKindStandard, ImagePaths: []string{"cover.jpg"}}
	if err := missingTitle.Validate(now); err == nil || !strings.Contains(err.Error(), "title is required") {
		t.Fatalf("Validate() error = %v", err)
	}
	missingContent := PublishRequest{Account: "creator-a", Title: "标题", Mode: PublishModeDraft, MediaKind: MediaKindStandard, ImagePaths: []string{"cover.jpg"}}
...
func TestPublishRequestValidateRejectsCrossMediaFields(t *testing.T) {
	now := shanghaiTime(2026, 4, 10, 12, 0, 0)
	standard := PublishRequest{Account: "creator-a", Title: "标题", Content: "正文", Mode: PublishModeDraft, MediaKind: MediaKindStandard, ImagePaths: []string{"cover.jpg"}, Live: LivePublishSource{ReportPath: "report.json", Resolved: &ResolvedLiveSource{Items: []ResolvedLiveItem{{PageName: "p01", PhotoPath: "/tmp/p01.jpg", VideoPath: "/tmp/p01.mov"}}}}}
	if err := standard.Validate(now); err == nil || !strings.Contains(err.Error(), "standard media request must not include live source") {
		t.Fatalf("Validate() error = %v", err)
	}
	live := PublishRequest{Account: "creator-live", Title: "标题", Content: "正文", Mode: PublishModeDraft, MediaKind: MediaKindLive, ImagePaths: []string{"cover.jpg"}, Live: LivePublishSource{ReportPath: "report.json", Resolved: &ResolvedLiveSource{Items: []ResolvedLiveItem{{PageName: "p01", PhotoPath: "/tmp/p01.jpg", VideoPath: "/tmp/p01.mov"}}}}}
